export class RetrocesoScannerForex {
    constructor({ swingLen = 15, emaLen = 50, extendBars = 200, labelOffsetBars = 4 } = {}) {
        ...
        for (let i = 0; i < bars.length; i++) {
            if (i < length - 1) {
                ...
                for (let j = 0; j < slice.length; j++) {
                    if (slice[j].high > maxVal) {
                        maxVal = slice[j].high;
                        maxIndex = j;
                    }
                }

                const bar = slice[maxIndex];
                result.push({ value: maxVal, time: bar.time });
            }
        }

        return result;
    }
    ta_lowest_series(bars, length) {
        const result = [];
        ...
        for (let i = 0; i < bars.length; i++) {
            if (i < length - 1) {
                ...
                let minIndex = 0;
                for (let j = 0; j < slice.length; j++) {
                    if (slice[j].low < minVal) {
                        minVal = slice[j].low;
                        minIndex = j;
                    }
                }

                const bar = slice[minIndex];
                result.push({ value: minVal, time: bar.time });
            }
        }
    ...
    EvaluaterBar(bar) {
        ...
        //Swing High/Low
        let lowestBar = this.ta_lowest_series(this.bars, this.swingLen)
        let highestBar = this.ta_highest_series(this.bars, this.swingLen);
        ...
        if (this.setupActivo) {
            ...
            this.ValidationCrown(this.fib95_, this.fib67_, this.fib35_, bar);
        }
        //Filtro Ema
        let bullBreak = bar.high > highestBar[highestBar.length - 2].value;
        let bearBreak = bar.low < lowestBar[lowestBar.length - 2].value;
        ...
        if (!this.setupActivo && bullBreak && alcistaPermitido) {
            this.fib0_ = bar.high;
            this.fib100_ = lowestBar[lowestBar.length - 1].value;
            this.fibTime_ = lowestBar[lowestBar.length - 1].time;
            ...
            if (!this.crown) {
                this.crown = (bar.close < this.fib67_);
            }
        ...
        if (this.esperando40 && this.esAlcista && !this.setupActivo) {
            if (this.bars[this.bars.length - 2].close > this.fib40_ && bar.low <= this.fib40_) {
                if (this.obEncontrado_ || this.fvgEncontrado_) {
                    ...
                    let tp2Pts = this.fib67_ - this.fib35_;
        ...
        if (!this.setupActivo && bearBreak && bajistaPermitido) {
            this.fib0_ = bar.low;
            this.fib100_ = highestBar[highestBar.length - 1].value;
            this.fibTime_ = highestBar[highestBar.length - 1].time;
    ...
    ValidationCrown(sl, pe, tp, lastBar) {
        let orientationUp = (sl > pe) ? true : false;
        if (orientationUp) {
            if ((lastBar.close > pe && this.crown == false)) {
                this.crown = true;
                return;
            }
            if (this.crown && this.setupActivo && (lastBar.close < tp || lastBar.close > sl)) {
                this.crown = false;
                this.setupActivo = false;
                this.clean();
            }
